...
int PushX, PushY, PushZ, nowx, nowy, BodyX[64], BodyY[64], BodyLen, x, y, i, FoodX, FoodY, Map[8][8], FoodShow = 0, TarX, TarY, p = 0, LimitTime, nowDire, PausePress;
//xyi是全局变量（用于遍历）
char NowMap[8];
int Dire = 3;//Dire表示蛇的方向：上下左右分别是1243
int DIN = 12, CS = 11, CLK = 10;//接口
int State = 0;//State表示状态：0代表开始界面；1代表游戏状态；2代表死亡；3代表通关;4代表暂停
int LimitTimeBase = 7;//限制多少-1次刷新之后接受蛇的输入
...
void loop()
{
	LimitTime = LimitTimeBase - BodyLen / 12;
	nowx = analogRead(A1);
	nowy = analogRead(A0);
	PushZ = digitalRead(2);
	if (PushZ)
